...
function hexToBuffer(hex) {
  if (hex.length % 2 !== 0) {
    this.$q.notify({
      type: 'warning',
      message: 'Expecting an even number of characters in the HEX',
      icon: 'link_off'
    });
  }
  ...
  if (bad) {
    this.$q.notify({
      type: 'warning',
      message: 'Found non-hex characters' + bad,
      icon: 'link_off'
    });
  }

  return new Uint8Array(
    hex.match(/[\dA-F]{2}/gi).map(item => {
...
function stringSeparator(string, count, split = " ") {
  return [...string].map(
    (d, i) => (i) % count === 0 ? split + d : d
  ).join('').trim();
}

export {charArrToHex, hexToBuffer, sleep, stringSeparator};
